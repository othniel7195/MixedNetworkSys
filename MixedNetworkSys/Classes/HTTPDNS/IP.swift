...
private let _ipModificationQueue = DispatchQueue(label: "com.mixedNet.HTTPDNS.ipModification", attributes: .concurrent)
...
private let IPV4_SOCKET_ADDRESS = "8.8.8.8"
private let IPV6_SOCKET_ADDRESS = "2001:4860:4860::8888"
private let SOCKET_ADDRESS_PORT = 53

extension IP {
  
  private static func constructIPError(errorCode: Int = -1, description: String? = nil) -> NSError {
    return NSError(domain: "com.mixdNet.httpDNS", code: errorCode, userInfo: [NSLocalizedDescriptionKey: description ?? "Unknown"])
  }
  ...
  private static func getIPV4() throws -> IP {
    ...
    inet_pton(AF_INET, IPV4_SOCKET_ADDRESS, &addr.sin_addr)
    ...
    inet_ntop(AF_INET, &localAddr, &buf, socklen_t(INET_ADDRSTRLEN))
  ...
  private static func getIPV6() throws -> IP {
    ...
    memset(&addr, 0, MemoryLayout.size(ofValue: addr))
    inet_pton(AF_INET6, IPV6_SOCKET_ADDRESS, &addr.sin6_addr)
    ...
    inet_ntop(AF_INET6, &localAddr, &buf, socklen_t(INET6_ADDRSTRLEN))
